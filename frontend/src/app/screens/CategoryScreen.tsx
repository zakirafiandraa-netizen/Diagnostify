...
export default function CategoryScreen() {
  ...
  return (
    <div className="flex flex-col min-h-screen lg:min-h-0">
      <NavBar title="Clinical System" onBack={goBack} />
      <StepBar current={1} />
        ...
        <motion.div variants={staggerList} initial="initial" animate="animate"
          ...
          {CATEGORIES.map((cat) => (
            <motion.button key={cat} variants={fadeUp} onClick={() => setSelectedCategory(cat)}
              ...
              className={`rounded-xl p-3 lg:p-4 text-xs lg:text-sm font-semibold text-center transition-all border active:scale-95 ${selectedCategory === cat
